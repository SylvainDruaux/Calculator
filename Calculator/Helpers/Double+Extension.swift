...
extension Double {
    ...
    func displayAdjusted(_ maxCharacters: Int) -> String {
        ...
        switch self {
        case _ where wholeCount > maxCharacters: // Example: maxCharacters = 10, number = 4166666666667 > 4.16667e12
            return self.scientificNotation(maxSignificantDigits)
        case _ where (wholeCount == maxCharacters && sign == .plus) || (wholeCount == maxCharacters + 1 && sign == .minus):
            numberFormatter.maximumFractionDigits = 0
            return numberFormatter.string(for: self) ?? String(self)
        case _ where abs(whole) == 0 && fraction != 0: // Number starts with -0. or 0. and also has fraction digits other than 0
            let factor = pow(10, maxFractionDigits)
            let num = self * (factor as NSDecimalNumber).doubleValue
            if num.whole == 0 { // Example: maxCharacters = 10, number = 0.0000000052 > 5.2e-9
                return self.scientificNotation(maxSignificantDigits)
            } else { // Example: maxCharacters = 10, number = 0.125555555555555 > 0.12555556
                numberFormatter.maximumFractionDigits = maxFractionDigits
                return numberFormatter.string(for: self) ?? String(self)
            }
        default: // Example: maxCharacters = 10, number = 1025.0055555555 > 1025.00556
            maxFractionDigits = maxCharacters - self.wholeCount - 1
            numberFormatter.maximumFractionDigits = maxFractionDigits
        }
        return numberFormatter.string(for: self) ?? String(self)
    }
    ...
    var wholeCount: Int {
        return String(self.whole).split(separator: ".").first!.count
    }
    
    var fractionCount: Int {
        let fractionStr = String(self.fraction)
        if fractionStr.contains(".") {
            return fractionStr.split(separator: ".").last!.count
        }
        return 0
    }
    ...
    var fractionVisibleCount: Int {
        return fractionVisible.count
    }
    
    /// Compare me to another Double
    func doubleEqual(_ num: Double) -> Bool {
        return fabs(self - num) < Double.ulpOfOne
    }
}
